-- Add latest_progress_image_urls to get_project_equipment_bundle so the grid can skip getLatestProgressImageUrlsBatch
-- when progressImagesLatestOnly is true (one map: equipment_id -> newest image_url).
...
BEGIN
  IF p_project_id IS NULL THEN
    RETURN jsonb_build_object(
      ...
      'vdcr_records', '[]'::jsonb,
      'users', '[]'::jsonb,
      'latest_progress_image_urls', '{}'::jsonb
    );
  END IF;
  ...
  SELECT coalesce(jsonb_object_agg(eid::text, u), '{}'::jsonb)
  INTO j_latest
  FROM (
    SELECT DISTINCT ON (equipment_id) equipment_id AS eid, image_url AS u
    FROM public.equipment_progress_images
    WHERE equipment_id = any (v_allowed_ids)
  ...
  RETURN jsonb_build_object(
    ...
    'vdcr_records', j_vdcr,
    'users', j_users,
    'latest_progress_image_urls', coalesce(j_latest, '{}'::jsonb)
  );
END;
$$;
